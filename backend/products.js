...
router.get('/', (req, res) => {
  db.all('SELECT Products.*, ProductImages.image_path FROM Products LEFT JOIN ProductImages ON Products.id = ProductImages.product_id', (err, rows) => {
    if (err) {
      console.error(err);
...
router.get('/:id', (req, res) => {
  ...
  db.get('SELECT Products.*, ProductImages.image_path FROM Products LEFT JOIN ProductImages ON Products.id = ProductImages.product_id WHERE Products.id = ?', [productId], (err, row) => {
    if (err) {
      console.error(err);
      return res.status(500).send('Erreur serveur');
    }
